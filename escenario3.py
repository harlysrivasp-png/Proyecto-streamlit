...
def escenario_3():
    ...
    if modelo_entrenado is not None:
        ...
        # BUG 1 CORREGIDO: el scaler estaba dentro del for, se ejecutaba
        # en cada iteración de columnas de texto en vez de una sola vez al final
        ...
        # BUG 2 CORREGIDO: todo el bloque de predicción estaba fuera del if,
        # causando UnboundLocalError cuando no se había subido modelo
        ...
        with col_nada_predi:
            ...
            # BUG 3 CORREGIDO: applymap deprecado en pandas 2.1+, reemplazado por map
            df_abandono = df_abandono.map(lambda x: "No Abandono" if x == 0 else "Si Abandono")
            st.write(df_abandono)

        with col_nada_pro:
            st.subheader('Probabilidad de predicción')
            etiquetas = ["No Abandono" if x == 0 else "Si Abandono"
                         for x in prediction_proba_modelo.argmax(axis=1)]
            probabilidades = np.where(
                np.array(etiquetas) == "No Abandono",
                prediction_proba_modelo[:, 0],
                prediction_proba_modelo[:, 1]
            )
            # BUG 4 CORREGIDO: se creaban dos columnas "Abandono" separadas al unir
            # df_abandono["Abandono"] con otra serie, generando duplicados.
            # Ahora se construye df_resultado directamente sin pasar por df_abandono
            df_resultado = pd.DataFrame({
                "Abandono": etiquetas,
                "Probabilidad Abandono": probabilidades
            })
        ...
        for index, row in df_resultado.iterrows():
            ...
            # BUG 5 CORREGIDO: la comparación era == "Si" pero el valor
            # real es "Si Abandono"
            if abandono == "Si Abandono":
        ...
        st.info("Por favor, sube un modelo (.pkl) para continuar.")
