...
def escenario2():
    ...
    st.markdown("""
    En este escenario se carga un archivo CSV con datos de estudiantes.
    El sistema utiliza un modelo previamente entrenado llamado `modelo_entrenado.pkl`
    para predecir si cada estudiante presenta riesgo de abandono académico.
    """)
    ...
    try:
        ...
        st.success("Modelo `modelo_entrenado.pkl` cargado correctamente.")

    except FileNotFoundError:
        st.error(
            "No se encontró el archivo `modelo_entrenado.pkl`. "
            "Debe entrenar y guardar el modelo en la carpeta principal del proyecto."
        )
        st.stop()
    ...
    datos_nuevos = st.sidebar.file_uploader(
        "Suba un archivo CSV con estudiantes",
        type=["csv"]
    )

    if datos_nuevos is None:
        st.info("Por favor, suba un archivo CSV para realizar la predicción.")
        st.stop()
    ...
    # Si el modelo tiene nombres de columnas guardados, se usan esos
    ...
    if hasattr(carga_modelo, "feature_names_in_"):
        ...
        if len(columnas_faltantes_modelo) > 0:
            st.error("Faltan columnas requeridas por el modelo después del preprocesamiento:")
